...
def get_mass_rockets(edl_system):
    ...
    m = edl_system['num_rockets']*(edl_system['rocket']
                                   ['structure_mass'] + edl_system['rocket']['fuel_mass'])
...
def get_mass_edl(edl_system):
    ...
    m = int(not(edl_system['parachute']['ejected']))*edl_system['parachute']['mass'] + \
        ...
        get_mass_rockets(
            edl_system) + edl_system['sky_crane']['mass'] + get_mass_rover(edl_system['rover'])
...
def get_cost_rover(rover):
    ...
    cost_chassis = (rover['chassis']['specific_strength'] /
                    100)**2*1000*rover['chassis']['mass']+50000
...
def get_cost_edl(edl_system):
    ...
    cost_fuel = edl_system['rocket']['initial_fuel_mass'] * \
        edl_system['rocket']['cost_per_kg']
    ...
    cost_parachute = edl_system['parachute']['cost_per_A'] * \
        np.pi*(edl_system['parachute']['diameter']/2)**2
...
def define_planet():
    ...
    def density(temperature, pressure): return pressure / \
        (0.1921*(temperature+273.15))  # [kg/m^3]
...
def define_edl_system():
    ...
    parachute = {'deployed': True,  # true means it has been deployed but not ejected
                 ...
                 # [$US/m^2] cost proportional to parachute area
                 'cost_per_A': 1e3,
    ...
    rocket = {'on': False,
              ...
              # [$US/kg] rocket cost per kg of fueld
              'cost_per_kg': 1500,
              # [kg] current fuel mass (<= initial)
              'fuel_mass': 230.0,
    ...
    sky_crane = {'on': False,            # true means lowering rover mode
                 # [m] altitude at which considered too low for safe rover touch down
                 'danger_altitude': 4.5,
                 # [m/s] speed at which rover would impact to hard on surface
                 'danger_speed': -1.0,
                 'mass': 35.0,           # [kg]
                 # [m^2] frontal area for drag calculations
                 'area': 16.0,
                 'Cd': 0.9,              # [-] coefficient of drag
                 # [m] max length of cable for lowering rover
                 'max_cable': 7.6,
...
def define_batt_pack(edl_system, battery_type, num_modules):
    ...
    if num_modules % 1 != 0 or num_modules <= 0:
        raise Exception(
            'define_batt_pack: num_modules must be a positive integer')
    ...
    if battery_type.lower() == 'LiFePO4'.lower():
        ...
        # $ (retail reflecting quantity discount)
        cost_per_module = 125000
...
def tau_dcmotor(omega, motor):
    ...
    if (type(omega) != int) and (type(omega) != float) and (not isinstance(omega, np.ndarray)):
        raise Exception(
            'First input must be a scalar or a vector. If input is a vector, it should be defined as a numpy array.')
...
def F_drag_descent(edl_system, planet, altitude, velocity):
    ...
    if not edl_system['heat_shield']['ejected']:
        ACd_body = np.pi * \
            (edl_system['heat_shield']['diameter']/2.0)**2 * \
            edl_system['heat_shield']['Cd']
    else:
        ACd_body = edl_system['sky_crane']['area'] * \
            edl_system['sky_crane']['Cd']
    ...
    if edl_system['parachute']['deployed'] and not edl_system['parachute']['ejected']:
        ACd_parachute = np.pi * \
            (edl_system['parachute']['diameter']/2.0)**2 * \
            edl_system['parachute']['Cd']
    else:
        ACd_parachute = 0.0
...
def F_gravity(terrain_angle, rover, planet):
    ...
    if (type(terrain_angle) != int) and (type(terrain_angle) != float) and (not isinstance(terrain_angle, np.ndarray)):
        ...
        # make the scalar a numpy array
        terrain_angle = np.array([terrain_angle], dtype=float)
    ...
    if max([abs(x) for x in terrain_angle]) > 75:
        raise Exception(
            'All elements of the first input must be between -75 degrees and +75 degrees')
    ...
    Fgt = np.array([-m*g*math.sin(math.radians(x))
                   for x in terrain_angle], dtype=float)
...
def F_rollingCorr(omega, terrain_angle, rover, planet, Crr):
    ...
    if (type(terrain_angle) != int) and (type(terrain_angle) != float) and (not isinstance(terrain_angle, np.ndarray)):
        raise Exception(
            'Second input must be a scalar or a vector. If input is a vector, it should be defined as a numpy array.')
    elif not isinstance(terrain_angle, np.ndarray):
        # make the scalar a numpy array
        terrain_angle = np.array([terrain_angle], dtype=float)
    elif len(np.shape(terrain_angle)) != 1:
        raise Exception(
            'Second input must be a scalar or a vector. Matrices are not allowed.')
    ...
    if max([abs(x) for x in terrain_angle]) > 75:
        raise Exception(
            'All elements of the second input must be between -75 degrees and +75 degrees')
    ...
    Fn = np.array([m*g*math.cos(math.radians(x))
                  for x in terrain_angle], dtype=float)  # normal force

    Frr_simple = -Crr*Fn  # simple rolling resistance

    Frr = np.array([math.erf(40*v_rover[ii]) * Frr_simple[ii]
                   for ii in range(len(v_rover))], dtype=float)
...
def motorW(v, rover):
    ...
    if (type(v) != int) and (type(v) != float) and (not isinstance(v, np.ndarray)):
        raise Exception(
            'v input must be a scalar or a vector. If input is a vector, it should be defined as a numpy array.')
    elif not isinstance(v, np.ndarray):
        v = np.array([v], dtype=float)  # make the scalar a numpy array
    elif len(np.shape(v)) != 1:
        raise Exception(
            'v input must be a scalar or a vector. Matrices are not allowed.')
...
def battenergy(t, v, rover):
    ...
    if (not isinstance(t, np.ndarray)):
        raise Exception(
            't input must be a scalar or a vector. If t is a vector, it should be defined as a numpy array.')
    ...
    if (not isinstance(v, np.ndarray)):
        raise Exception(
            'v input must be a scalar or a vector. If v is a vector, it should be defined as a numpy array.')
    elif len(np.shape(v)) != 1:
        raise Exception(
            'v input must be a scalar or a vector. Matrices are not allowed.')
    ...
    # calculate torque (used for efficiency info)
    tau = tau_dcmotor(omega, rover['wheel_assembly']['motor'])
...
def rover_dynamics(t, y, rover, planet, experiment):
    ...
    if (not isinstance(y, np.ndarray)) or (len(y) != 2):
        ...
        # this will turn the column vector into a row
    ...
    omega = motorW(v, rover)
    alpha_fun = interp1d(experiment['alpha_dist'].ravel(
    ), experiment['alpha_deg'].ravel(), kind='cubic', fill_value="extrapolate")
...
def end_of_mission_event(end_event):
    ...
    def distance_left(t, y): return mission_distance - y[1]
    distance_left.terminal = True

    def time_left(t, y): return mission_max_time - t
    time_left.terminal = True
    time_left.direction = -1

    def velocity_threshold(t, y): return y[0] - mission_min_velocity
...
def simulate_rover(rover, planet, experiment, end_event):
    ...
    # Main Code
    def fun(t, y): return rover_dynamics(t, y, rover,
                                         planet, experiment)  # differential equation
    ...
    # t_eval=(np.linspace(0, 3000, 1000)))  # need a stiff solver like BDF
    sol = solve_ivp(fun, t_span, y0, method='BDF', events=events, max_step=1.0)
    ...
    telemetry = {'Time': sol.t,
                 ...
                 'velocity': sol.y[0, :],
                 'position': sol.y[1, :],
                 # Matlab version had an integration of velocity over time, but if velocity must be positive (defined by min_velocity), then the final position is the distance traveled
...
def edl_events(edl_system, mission_events):
    ...
    def event0(t, y): return y[1] - mission_events['alt_heatshield_eject'] - \
        int(edl_system["heat_shield"]["ejected"])*999999
    event0.terminal = True
    event0.direction = -1

    def event1(t, y): return y[1] - mission_events['alt_parachute_eject'] - \
        int(edl_system["parachute"]["ejected"])*999999
    event1.terminal = True
    event1.direction = -1

    def event2(t, y): return y[1] - mission_events['alt_rockets_on'] - \
        int(edl_system["rocket"]["on"])*999999
    event2.terminal = True
    event2.direction = -1

    def event3(t, y): return y[1] - mission_events['alt_skycrane_on'] - \
        int(edl_system["sky_crane"]["on"])*999999
    event3.terminal = True
    event3.direction = -1

    def event4(t, y): return y[2]
    event4.terminal = True
    event4.direction = -1

    def event5(t, y): return y[1]
    event5.terminal = True
    event5.direction = -1

    def event6(t, y): return y[0] - 3*edl_system['speed_control']['target_velocity'] + \
        int(edl_system["speed_control"]["on"])*999999
    event6.terminal = True
    event6.direction = 1

    def event7(t, y): return y[1] - 1.2*mission_events['alt_skycrane_on'] - \
        int(edl_system["position_control"]["on"])*999999
    event7.terminal = True
    event7.direction = -1

    def event8(t, y): return y[1] + y[6]
...
def edl_dynamics(t, y, edl_system, planet):
    ...
    F_ext = F_gravity_descent(edl_system, planet) + F_buoyancy_descent(edl_system,
                                                                       planet, altitude_edl) + F_drag_descent(edl_system, planet, altitude_edl, vel_edl)
    ...
    if edl_system['rocket']['on'] and not(edl_system['speed_control']['on']) and not(edl_system['position_control']['on']):
        ...
        F_thrust = 0.9*edl_system['rocket']['max_thrust'] * \
            edl_system['num_rockets']  # Thrust from rockets
        ...
        num = (Kp*e_vel + Kd*(F_ext/edl_mass) +
               Ki*ei_vel) - edl_mass*planet['g']
        den = (1-Kd/edl_mass)
        F_thrust = num/den

        # this ensures we never try to reverse thrust (which is impossible in
        # this system)
        F_thrust = max(edl_system['num_rockets'] *
                       edl_system['rocket']['min_thrust'], F_thrust)

        # this checks for saturation of thrust (more than 100# of what rockets
        # can deliver)
        F_thrust = min(
            F_thrust, edl_system['num_rockets']*edl_system['rocket']['max_thrust'])
        ...
        F_thrust = edl_system['num_rockets'] * \
            (Kp*e_pos + Kd*dedt_pos + Ki*ei_pos) - planet['g']*edl_mass

        # enforces a minimum thrust level since we cannot thrust downward
        F_thrust = max(edl_system['rocket']['min_thrust']
                       * edl_system['num_rockets'], F_thrust)
    ...
    for i in range(9):
        ...
        if event == 0:  # heat shield eject
            if TE[i].size != 0:
                ...
                if not (edl_system["heat_shield"]["ejected"]):
                    ...
                    if ITER_INFO:
                        print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                            'Ejecting heat shield at', 't =', time, 'altitude =', altitude, 'speed =', speed))
                    y0 = Y[:, -1]
                    # y0[1] = y0[1]  # - 0.001
        ...
        if event == 1:  # parachute shield eject
            if TE[i].size != 0:
                ...
                if not (edl_system["parachute"]["ejected"]):
                    ...
                    if ITER_INFO:
                        print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                            'Ejecting parachute at', 't =', time, 'altitude =', altitude, 'speed =', speed))
                    y0 = Y[:, -1]
                    # y0[1] = y0[1] #- 0.001
        ...
        if event == 2:  # turning on rockets, but w/o control
            if TE[i].size != 0:
                ...
                if not (edl_system["rocket"]["on"]):
                    ...
                    if ITER_INFO:
                        print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                            'Turning on rockets at', 't =', time, 'altitude =', altitude, 'speed =', speed))
                    y0 = Y[:, -1]
                    # y0[1] = y0[1] # - 0.001

        # turn on sky crane if we're low enough (triggers event 4) and we're under a position-controlled regime
        if event == 3:
            if TE[i].size != 0:
                time = TE[i][0]
                altitude = YE[i][0, 1]
                ...
                if not (edl_system["sky_crane"]["on"]) and edl_system["position_control"]["on"]:
                    ...
                    if ITER_INFO:
                        print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                            'Turning on sky crane at', 't =', time, 'altitude =', altitude, 'speed =', speed))
                y0 = Y[:, -1]
        ...
        if event == 4:  # we are out of rocket fuel!
            if TE[i].size != 0:
                ...
                if edl_system["rocket"]["on"]:
                    ...
                    if ITER_INFO:
                        print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                            'Ran out of rocket fuel at', 't =', time, 'altitude =', altitude, 'speed =', speed))
                    #y0 = Y[:, -1]
        ...
        if event == 5:  # edl crashed before sky crane is activated
            if TE[i].size != 0:
                ...
                if ITER_INFO:
                    print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                        'EDL SYSTEM CRASHED INTO MARS AT', 't =', time, 'altitude =', altitude, 'speed =', speed))
                y0 = []
                TERMINATE_SIM = True
        ...
        if event == 6:  # still descending, but slow enough to turn on speed controller
            if TE[i].size != 0:
                ...
                if not (edl_system["speed_control"]["on"]) and not (edl_system["position_control"]["on"]):
                    ...
                    if ITER_INFO:
                        print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                            'Turning on speed control at', 't =', time, 'altitude =', altitude, 'speed =', speed))
                else:
                    edl_system['speed_control']['on'] = True
                    if ITER_INFO:
                        print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                            'Trouble at', 't =', time, 'altitude =', altitude, 'speed =', speed))
                y0 = Y[:, -1]
                y0[3] = 0
                y0[4] = 0
                # y0[0] = y0[0] #+ 0.01
        ...
        if event == 7:
            if TE[i].size != 0:
                ...
                if not (edl_system["position_control"]["on"]) and edl_system['speed_control']['on']:
                    ...
                    if ITER_INFO:
                        print("{:<30} {:<3} {:<8.4f} [s], {:<10} {:<9.4f} [m], {:<7} {:<9.4f} [m/s]".format(
                            'Turning on altitude control at', 't =', time, 'altitude =', altitude, 'speed =', speed))
                        y0 = Y[:, -1]
                        y0[3] = 0
                        y0[4] = 0
                        # y0[1] = y0[1] # - 0.00001
                    ...
                    if ITER_INFO:
                        print(
                            "SYSTEM FAIL: SPEED CONTROL DID NOT ACTIVATE PRIOR TO ALTITUDE CONTROL")
                    TERMINATE_SIM = True
                    y0 = []
                    #edl_system = redefine_edl_system(edl_system)

        # we've determined the rover position is at 0 altitude (on the ground)
        if event == 8:
            if TE[i].size != 0:
                time = TE[i][0]
                altitude = YE[i][0, 1]
...
def simulate_edl(edl_system, planet, mission_events, tmax, ITER_INFO):
    ...
    y0 = np.array([edl_system['velocity'],
                   edl_system['altitude'],
                   edl_system['rocket']['initial_fuel_mass'] *
                   edl_system['num_rockets'],
                   0,
                   0,
                   0,
                   0])
    ...
    while not(TERMINATE_SIM):
        ...
        def fun(t, y): return edl_dynamics(t, y, edl_system, planet)
        sol = solve_ivp(fun, tspan, y0, method='DOP853',
                        events=events, max_step=0.1)
        t_part = sol.t
        Y_part = sol.y
        TE = sol.t_events
        YE = sol.y_events
        ...
        # TERMINATE_SIM flag.
        # print(YE)
        [edl_system, y0, TERMINATE_SIM] = update_edl_state(
            edl_system, TE, YE, Y_part, ITER_INFO)
...
def obj_fun_time(x, edl_system, planet, mission_events, tmax, experiment, end_event):
    ...
    [time_edl_run, _, edl_system] = simulate_edl(
        edl_system, planet, mission_events, tmax, False)
...
def constraints_edl_system(x, edl_system, planet, mission_events, tmax, experiment, end_event, min_strength, max_rover_velocity, max_cost, max_batt_energy_per_meter):
    ...
    constraint_distance = (end_event['max_distance']-edl_system['rover']
                           ['telemetry']['distance_traveled'])/end_event['max_distance']
    #
    # The chassis must be strong enough to survive the landing
    chassis_strength = edl_system['rover']['chassis']['mass'] * \
        edl_system['rover']['chassis']['specific_strength']
    constraint_strength = -(chassis_strength-min_strength)/min_strength
    #
    # The battery must not run out of charge
    constraint_battery = (edl_system['rover']['telemetry']['energy_per_distance'] -
                          max_batt_energy_per_meter)/max_batt_energy_per_meter
    #
    # The touchdown speed of the rover must not be too much (or else damage may occur)
    constraint_velocity = (
        abs(edl_system['velocity'])-abs(max_rover_velocity))/abs(max_rover_velocity)
    #
    ...
    # **
    c = [constraint_distance, constraint_strength,
         constraint_velocity, constraint_cost, constraint_battery]
...
def obj_fun_plot(x, edl_system, planet, mission_events, tmax, experiment, end_event):
    ...
    edl_system['rover']['wheel_assembly']['speed_reducer']['diam_gear'] = x[3]
    #
    [time_edl_run, Y_edl_run, edl_system] = simulate_edl(
        edl_system, planet, mission_events, tmax, False)
    time_edl = time_edl_run[-1]
    #
    # *****************
